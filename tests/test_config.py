...
def test_retention_defaults():
    s = _settings()
    assert s.max_tabs_per_space == 50  # bounded recent-tab cache
    assert s.max_history_per_space is None  # history unbounded by default
...
def test_blank_retention_env_is_treated_as_unset(monkeypatch):
    ...
    # must NOT crash startup — it means "unbounded".
